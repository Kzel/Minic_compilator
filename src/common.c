...
void parse_args(int argc, char ** argv) {
    ...
    {
        char* in = argv[i];
        int longeur = strlen(in);

        if(in[longeur-2]=='.' && in[longeur-1]=='c')
        {
        infile = argv[i];
        }
        else 
        {
            if(strcmp(argv[i],"-b")==0){
                ...
                printf("Usage: ./minicc_ref <options> <infile>\n");
                printf("    <infile>      : Input file to compile (mandatory)\n");
                printf("option:\n");                   
                printf("    -o <filename> : Set out file name (defaut = out.s).\n");            
                printf("    -t <int>      : Set trace level between 0 and 5 (0 = no trace; 5 = all traces. defaut = 0).\n");
                printf("    -r <int>      : Set maximum number of registers to use (default = 8).\n");
                printf("    -s            : Stop compilation after the syntax analysis (defaut = no).\n");            
                printf("    -v            : Stop compilation after the verification pass (default = no).\n");
                printf("    -h            : Display this help\n");
                printf("Option '-s' and '-v' are incompatible\n");            
                exit(0);
            }
                ...
                {
                    printf("The name of assembly file is incorrect.\n");
                    exit(-1);
                }
                exit(0);
            }
                ...
                i++;
                int nbregistre = atoi(argv[i]);
                if(nbregistre >= 4 && nbregistre <= 8)
                {
                    set_max_registers(nbregistre);
                }
                else
                {
                    break;
